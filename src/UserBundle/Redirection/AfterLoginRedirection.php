...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        if (in_array('ROLE_PATRON', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('patron_homepage'));
        // otherwise, if is a commercial user we redirect to the crm area
        elseif (in_array('ROLE_SECRETAIRE', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('secretaire_homepage'));
        elseif (in_array('ROLE_ARTISAN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('artisan_homepage'));
